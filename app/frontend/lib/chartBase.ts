// The shared half of `chart.coffee`.
...
export interface ChartInput {
  ...
  /** The tooltip's own heading — a month on the dashboard, a full date on a project. */
  dateLabel?: (label: string) => string
...
// `baseChartOptions`: everything both charts share — the categories, the
// series with their dashed continuation, the shared tooltip with its
// category-wide crosshair, and no legend.
export function baseChartOptions(input: ChartInput): Record<string, unknown> {
  ...
  const categories: ChartCategory[] = input.labels.map((label) => ({
    ...
    date: (input.dateLabel ?? input.monthLong)(label),
  }))
  ...
  return {
    ...
    tooltip: {
      ...
      style: {padding: 0},
      shared: true,
      // A category-wide crosshair, which Highcharts draws as a band across
      // the whole month rather than a hairline.
