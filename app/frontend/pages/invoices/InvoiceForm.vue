    ...
    <div class="flex flex-wrap items-start gap-4">
      ...
      <div class="max-md:w-full">
        <RouterLink :to="{ name: 'invoices' }" data-test="back">
          <UiButton as="span" class="max-md:w-full">{{ t("invoiceForm.back") }}</UiButton>
        </RouterLink>
      </div>
    </div>
    ...
    <form v-else class="mt-4" @submit.prevent="save">
      <div class="grid gap-4 md:grid-cols-3">
        <!-- `belongs_to :project` is required, so an invoice never has none.
             The empty entry is the "nothing picked yet" of a new invoice; on
             an existing one it would promise a clearing the server cannot
             carry out. -->
        ...
        <template #body>
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2">
            <UiButton type="button" data-test="add-position" @click="addRow">
              <i class="fa fa-plus"></i> {{ t("invoiceForm.addPosition") }}
            </UiButton>
