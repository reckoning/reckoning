...
function respond(requests: AxiosRequestConfig[], options: Options) {
  AXIOS_INSTANCE.defaults.adapter = async (config) => {
    ...
    else if (url.includes("/projects/")) data = {id: PROJECT_ID, name: "Narendra 3", rate: "90.0", workflowState: "active", tasks: []}
    else if (url.includes("/projects")) data = [{id: PROJECT_ID, name: "Narendra 3", label: "Narendra 3", workflowState: "active", tasks: []}]
